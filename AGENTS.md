  ...
  data/
    ...
    health/                ← HealthConnectRepository (read-only; lazy client; PERMISSIONS companion set);
                              ...
                              **Aggregates must be permission-scoped** — Health Connect fails the
                              ...
                              estimates); HealthVitalsTest pins it. VitalRange.kt: the usual range
                              Body & Vitals draws and flags against (VitalRangeTest).
...
The F1 countdown's digits roll over (`RollingText`, components/RollingText.kt): each changed character
slides up out of its own clipped box as the new one rises in from below, as a countdown ticks on iOS. Specs
in `MacroMotion.CountdownRoll`; the digit style uses tabular figures so nothing shifts.
...
```
A card missing from a saved order (one added in an update) is slotted in after the card that precedes it by default (`parseWidgetConfig`), not appended. `DAILY_HEALTH` is Today, the hero: one sentence on the day (`dailyHeadline`: readiness from last night's sleep + HRV + resting HR against the person's own 30-day baseline, then the step pace), Apple Watch-style gradient `ActivityRings` with Apple Fitness readouts in each ring's colour (`RingReadouts`), and today's steps drawn against a usual day (`StepPaceChart`; `readUsualHourlySteps` is one hourly aggregate over 14 days, and the pure maths in data/health/StepPace.kt is pinned by StepPaceTest). The day's other numbers are **`BODY_STATS`**, titled "Today's readings" (`TodaysReadingsSection`), just below it: one Apple Health summary row per metric (activity, then heart, then breathing), each today's number big with its unit, one plain line against yesterday (`readingComparison`: a total only compares once it has passed yesterday's; pinned by TodaysReadingsTest) and the week beside it (bars for totals, `WeekDots` for rates). Metrics with nothing yet today fold into one line at the foot. Each section's name says what it holds, and the editor uses the same names: Today, Today's readings, Sleep, Activities, Body & Vitals, Trends, then the food group (Food today, Log food, Food trends, Food log). A section's header colour is its category's tone; sections that span categories (Today's readings, Trends) use `Primary`. **`SLEEP`** is last night in full (the night on a 24-hour bedtime dial after the iOS Clock app, `SleepClockDial`, in its stage colours; score, bedtime and wake beside it; stage mix, hypnogram) over a two-week schedule chart; tap or drag a night to read it, with bedtime/wake regularity and sleep debt below. **`VITALS`** (Body & Vitals) covers every measure Health Connect has (HRV, resting HR, sleeping HR, daily heart rate, weight + BMI, body fat, lean mass, body water, bone mass, VO₂ max, blood pressure, glucose, SpO₂, respiration, body and skin temperature, hydration, resting energy), plus estimates marked "Est." where nothing writes the measure (resting HR, VO₂ max, resting energy). It opens as Apple's Vitals: a sentence (`vitalsHeadline`) and a column per overnight vital, its usual range a grey capsule and the latest reading a dot (blue inside; outside, green the good way and amber the other). The usual range is data/health/VitalRange.kt (`usualRange`: 30-day mean ± one SD, never narrower than `minUsualHalfWidth`; `standingOf`; pinned by VitalRangeTest). Then the vitals as list rows (name, number, "Typical · Usual 61 bpm") and the body measures as summary rows with their trend; a row or a column opens its scrubbable trend with that same range as a band. Shared pieces (chips, delta arrows that know which way is good, sparklines, stat tiles) live in `health/HealthUiKit.kt`; the iOS-style rows (`HealthReadingRow`, `HealthValueText` with tabular figures, `InsetHairline` starting where the text does, `HealthGroupLabel`, `HealthPromptRow`, `EstimateBadge`) in `health/HealthReadingRow.kt`. Each section is one card (`HealthSection` in health/HealthSection.kt, the same `MacroCard` Home uses) opened by a `HealthHeader` whose icon and title carry the category's colour, as Apple Health heads its summary. Nothing inside a card gets a box of its own: no inset wells or outlines behind charts, stats or rows; lists and grids split with `Hairline`s, and stat tiles are a label over a number. Six category tones in Color.kt (`HealthActivityTone`, `HealthHeartTone`, `HealthSleepTone`, `HealthBodyTone`, `HealthVitalsTone`, `HealthNutritionTone`; the per-measure names alias them) colour headers, icons, rings and chart lines. Numbers are white and labels grey (Activities excepted, below), a selected chip is a neutral fill, a worse delta is grey (never red), and a band caption only takes a colour when the reading needs attention. The Sleep card keeps a picked night by its date, and only until a newer night syncs, so the card always comes back to last night. Home's **`BODY_STATS`** is `HealthGlanceCard` (health/HealthGlanceCard.kt): the same rings and readouts, then one chart with a sentence over it (`GlanceChart`): Steps (the pace chart and `paceLine`), Heart (`HeartDayChart`: today's heart rate in quarter hours from `heartRateCurve`, pinned by HeartRateDayTest, over the resting line), Move (`readHourlyActiveCalories` against the Move goal) and Sleep (`SleepStagesStrip`, last night's stages). It moves on to the next every `MacroMotion.GlanceCycle.INTERVAL_MS` while on screen and the app is in front, until a chip is tapped, and opens on Sleep before 11:00. Every chart is drawn at one height and each sentence keeps to a line, so the card never changes size on Home. Readings follow as a line of text. The tab pulls to refresh (`HealthViewModel.refresh()`), and Trends pages back `HealthViewModel.MAX_WEEKS_BACK` weeks with week-over-week deltas. **`ACTIVITIES`** lists this month's and last month's workouts synced through Health Connect (Garmin Connect, Google Fit, Samsung Health, Strava, and others) the way Apple Fitness does: the last seven days as three big numbers (exercise minutes against the WHO's 150, workouts, distance) over a bar a day, the latest workout (`pickFeaturedActivity`, the newest) with a two-column detail grid, its heart rate and laps, then the rest as rows (sport badge, headline number, day) with **Show N more** revealing them in a scroll box. A workout's numbers take the colour of what they measure, as Fitness paints them: time and steps in the Activity tone, distance and pace blue, energy and heart rate red, climb green. A workout's heart rate loads through `HealthViewModel.onActivityExpanded()` when its row is opened. `WEEK_AT_A_GLANCE` is the Macro Trends widget (7/14/30-day nutrition chart + per-day food logs), moved from the former History tab.
